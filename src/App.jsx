...
const goals = [
  ...
  ["Database", "500-1,000 new contacts added to email list", "Compounds beyond Q1 - fuels the next campaign at near-zero cost"],
  ["Audience", "500+ followers on the Pitchr LinkedIn brand page by end of August", "Owned reach that compounds beyond Q1. Reduces dependency on paid for every future campaign."]
];
...
const kpiRows = [
  ["Launch month (June)", "15-30", "12-15", "+300"],
  ["Mid-launch (July)", "15-30", "6-8", "+250"],
  ["Late-launch (August)", "20-30", "6-10", "+200"],
  ["Total Q1", "50-90", "24+", "+750"]
];
...
function Timelines() {
  ...
  const [activeGroup, setActiveGroup] = useState(timelineGroups[0].group);
  const selectedGroup = timelineGroups.find((group) => group.group === activeGroup) || timelineGroups[0];
  return <Shell eyebrow="5. Timeline" title="Grouped execution Gantt." intro="Each workstream is split into its own Gantt tab to keep the chart compact.">{validationErrors.length > 0 && <div className="mt-5 rounded-2xl border border-red-400/40 bg-red-400/10 p-4 text-xs font-bold text-red-200"><p>Timeline data check:</p><BulletList items={validationErrors} /></div>}<div className="mt-6 flex gap-2 overflow-x-auto rounded-full border border-white/10 bg-white/[0.03] p-1">{timelineGroups.map((group) => <button key={group.group} type="button" onClick={() => setActiveGroup(group.group)} className={`whitespace-nowrap rounded-full px-4 py-2 text-[11px] font-bold transition ${activeGroup === group.group ? "bg-[#A6FF00] text-black" : "text-white/45 hover:bg-white/5 hover:text-white"}`}>{group.group}</button>)}</div><div className="mt-6 overflow-x-auto rounded-[1.5rem] border border-white/10 bg-[#111111]"><div className="grid min-w-[1180px] grid-cols-[220px_260px_1fr] border-b border-white/10"><div className="border-r border-white/10 px-4 py-3 text-center text-[12px] font-black uppercase leading-4 tracking-tight text-white">Pitchr Launch</div><div className="border-r border-white/10 px-4 py-3 text-[10px] font-black uppercase tracking-[0.14em] text-white/45">Tactic</div><div className="grid grid-cols-5">{monthHeaders.map(({ month, weeks, align }) => <div key={month} className={`border-r border-white/10 px-2 py-3 last:border-r-0 ${align}`}><div className="text-sm font-black leading-none text-white">{month}</div><div className="mt-2 grid grid-cols-4 text-[9px] font-black leading-none text-white/45">{weeks.map((week, index) => <span key={`${month}-${index}`}>{week}</span>)}</div></div>)}</div></div><TimelineGroup group={selectedGroup} /></div></Shell>;
}
